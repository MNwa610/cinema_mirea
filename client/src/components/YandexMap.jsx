...
function YandexMap({ from, to, markers = [], showRoute = false, routeMode = 'car' }) {
      ...
      try {
        ...
        if (Array.isArray(markers) && markers.length > 0) {
          markers.forEach((m) => {
            ...
            if (!firstMarkerPlacemark) firstMarkerPlacemark = placemark
        ...
        if (showRoute && fromPoint && toPoint) {
          const isMultiRoute = routeMode !== 'car'
          const routingMode =
            routeMode === 'walking'
              ? 'pedestrian'
              : routeMode === 'public'
          ...
          if (!showRoute && !fromPoint && !toPoint && firstMarkerPlacemark) {
            setTimeout(() => {
              try {
                firstMarkerPlacemark.balloon.open()
              } catch (e) {
              }
            }, 0)
          }
  ...
  }, [from, to, JSON.stringify(markers), showRoute, routeMode])
